...
func TestReElection2A(t *testing.T) {
	...
	DPrintf(dWarn, "S%v dissconnected", leader1)
	...
	cfg.connect(leader1)
	DPrintf(dWarn, "S%v connected", leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect(leader2)
	DPrintf(dWarn, "S%v dissconnected", leader2)
	cfg.disconnect((leader2 + 1) % servers)
	DPrintf(dWarn, "S%v dissconnected", (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
...
// each command is sent to each clientEnd just once.
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf(dTrace, "S%v line 380 leader disconnected", leader1)
	cfg.disconnect(leader1)
	...
	leader2 := cfg.checkOneLeader()
	DPrintf(dTrace, "S%v line 394 leader disconnected", leader2)
	cfg.disconnect(leader2)

	// old leader connected again
	DPrintf(dTrace, "S%v line 398 leader %v connected", leader2, leader1)
	cfg.connect(leader1)
	cfg.one(104, 2, true)

	// all together now
	DPrintf(dTrace, "S%v line 403 leader %v connected", leader2, leader2)
	cfg.connect(leader2)
